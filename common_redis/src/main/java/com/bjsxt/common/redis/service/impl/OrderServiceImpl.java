...
public class OrderServiceImpl implements OrderService {
    ...
    @Value("${init_item_id_key}")
    private Long init_item_id_key;
    ...
    public Long selectOrderId() {
        ...
        //如果id值不存在，将默认值set到redis中
        if (value==null&&value<=0){
            this.redisTemplate.opsForValue().set(this.order_item_id_key, this.init_item_id_key);
        }
        //通过redis的自增长将默认值做自增处理
